...
func (h *SnapshotHandler) Snapshot(c *gin.Context) {
	...
	if err != nil {
		slog.Warn("Snapshot: no analysis found", "domain", domain, "error", err)
	...
	if err := json.Unmarshal(analysis.FullResults, &results); err != nil {
		slog.Error("Snapshot: failed to unmarshal results", "domain", domain, "error", err)
	...
	snapshot += fmt.Sprintf("; Integrity: SHA-256 hash of this document: %s\n", hashHex)
	snapshot += fmt.Sprintf("; ============================================================\n")
...
func GenerateObservedSnapshot(domain string, results map[string]any, appVersion string) string {
	...
	mxRecords := extractStringSlice(basic, "MX")
	if len(mxRecords) > 0 {
		sb.WriteString("; --- MX Records ---\n")
		ttl := getTTL(ttls, "MX")
		for _, rec := range mxRecords {
			sb.WriteString(fmt.Sprintf("%s    %s    IN    MX    %s\n", fqdn, ttl, rec))
		}
		sb.WriteString("\n")
	}

	nsRecords := extractStringSlice(basic, "NS")
	if len(nsRecords) > 0 {
	...
	soaRecords := extractStringSlice(basic, "SOA")
	if len(soaRecords) > 0 {
		sb.WriteString("; --- SOA Record ---\n")
		ttl := getTTL(ttls, "SOA")
		for _, rec := range soaRecords {
			sb.WriteString(fmt.Sprintf("%s    %s    IN    SOA    %s\n", fqdn, ttl, rec))
		}
		sb.WriteString("\n")
	}

	caaRecords := extractStringSlice(basic, "CAA")
	if len(caaRecords) > 0 {
		sb.WriteString("; --- CAA Records ---\n")
		ttl := getTTL(ttls, "CAA")
		for _, rec := range caaRecords {
			sb.WriteString(fmt.Sprintf("%s    %s    IN    CAA    %s\n", fqdn, ttl, rec))
		}
		sb.WriteString("\n")
	}

	srvRecords := extractStringSlice(basic, "SRV")
	...
	cnameRecords := extractStringSlice(basic, "CNAME")
	sb.WriteString("; --- CNAME Records ---\n")
	if len(cnameRecords) > 0 {
		ttl := getTTL(ttls, "CNAME")
