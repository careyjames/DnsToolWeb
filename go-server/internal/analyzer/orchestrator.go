...
func (a *Analyzer) AnalyzeDomain(ctx context.Context, domain string, customDKIMSelectors []string) map[string]any {
	...
	results := map[string]any{
		...
		"domain_status_message":  domainStatusMessage,
...
func (a *Analyzer) buildNonExistentResult(domain, status string, statusMessage *string) map[string]any {
	...
	return map[string]any{
		...
		"domain_status_message":  statusMessage,
		...
		"posture":                map[string]any{"score": 0, "grade": "N/A", "label": "Non-existent Domain", "issues": []string{"Domain does not exist or is not delegated"}, "color": "secondary"},
	}
}
