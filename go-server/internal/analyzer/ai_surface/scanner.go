...
func (s *Scanner) checkLLMSTxt(ctx context.Context, domain string, evidence *[]Evidence) map[string]any {
        ...
        for _, scheme := range []string{"https", "http"} {
                url := fmt.Sprintf("%s://%s/.well-known/llms.txt", scheme, domain)
                ...
                if resp.StatusCode == http.StatusOK {
                        ...
                        if len(content) > 10 {
                                ...
                                *evidence = append(*evidence, Evidence{
                                ...
                                slog.Info("AI Surface: llms.txt found", "domain", domain)
                        }
                        break
                }
        }
        ...
        for _, scheme := range []string{"https", "http"} {
                fullURL := fmt.Sprintf("%s://%s/.well-known/llms-full.txt", scheme, domain)
                resp, err := s.HTTP.Get(ctx, fullURL)
                if err != nil {
                        continue
                }
                defer resp.Body.Close()
