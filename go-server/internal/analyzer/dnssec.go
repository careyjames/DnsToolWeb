...
func (a *Analyzer) AnalyzeDNSSEC(ctx context.Context, domain string) map[string]any {
	...
	if len(dsRecords) > 0 {
		...
		if len(parts) >= 2 {
			if algNum, err := strconv.Atoi(parts[1]); err == nil {
				algorithm = &algNum
	...
	if adFlag {
		...
		var parentAlgo *int
		var parentAlgoName *string

		if parentZone != "" {
			parentDS := a.DNS.QueryDNS(ctx, "DS", parentZone)
			if len(parentDS) > 0 {
				parts := strings.Fields(parentDS[0])
				if len(parts) >= 2 {
					if algNum, err := strconv.Atoi(parts[1]); err == nil {
						parentAlgo = &algNum
						if name, ok := algorithmNames[algNum]; ok {
