...
func TestComputeRescanInterval_FloorEnforced(t *testing.T) {
        ...
        if result != freshnessFloorSeconds {
                t.Errorf("expected floor %d, got %d", freshnessFloorSeconds, result)
        }
}

func TestComputeRescanInterval_CeilingEnforced(t *testing.T) {
        result := computeRescanInterval(100000, 0)
        if result != freshnessCeilingSeconds {
                t.Errorf("expected ceiling %d, got %d", freshnessCeilingSeconds, result)
        }
}
...
func TestBuildFreshnessMatrix_WithResolverTTLs(t *testing.T) {
        ...
        result := BuildFreshnessMatrix(resolverTTL, authTTL)

        entries, ok := result["entries"].([]FreshnessEntry)
        if !ok {
                t.Fatal("expected entries to be []FreshnessEntry")
        }
...
func TestBuildFreshnessMatrix_FallsBackToAuthTTL(t *testing.T) {
        resolverTTL := map[string]uint32{}
        authTTL := map[string]uint32{
                "A": 120,
        }
        ...
        result := BuildFreshnessMatrix(resolverTTL, authTTL)
        entries := result["entries"].([]FreshnessEntry)
...
func TestBuildFreshnessMatrix_EmptyTTLs(t *testing.T) {
        result := BuildFreshnessMatrix(map[string]uint32{}, map[string]uint32{})
        ...
        for _, entry := range entries {
                ...
                if entry.RescanAfter < freshnessFloorSeconds {
                        t.Errorf("%s: rescan below floor", entry.RecordType)
                }
        }
}

func TestBuildFreshnessMatrix_AllRecordTypesPresent(t *testing.T) {
        result := BuildFreshnessMatrix(map[string]uint32{}, map[string]uint32{})
        entries := result["entries"].([]FreshnessEntry)
        ...
        for rt := range expected {
                if !found[rt] {
                        t.Errorf("missing record type %s in freshness matrix", rt)
                }
        }
}
