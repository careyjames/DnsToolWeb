...
func (a *Analyzer) AnalyzeDANE(ctx context.Context, domain string, mxRecords []string) map[string]any {
        ...
        if mxCapability != nil {
                ...
                if inbound {
                        providerContext["deployment_guidance"] = "Your MX provider supports DANE. Publish TLSA records for your MX hosts to enable DANE protection."
                } else {
                        alt, _ := mxCapability["alternative"].(string)
                        if alt != "" {
                                providerContext["deployment_guidance"] = fmt.Sprintf("Your MX provider does not support DANE inbound. Consider deploying %s as an alternative for transport security.", alt)
                        } else {
                                providerContext["deployment_guidance"] = "Your MX provider does not support DANE inbound. Consider MTA-STS as an alternative for transport security."
                        }
                }
                baseResult["provider_context"] = providerContext
        }
