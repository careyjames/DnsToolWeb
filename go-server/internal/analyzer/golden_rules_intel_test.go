...
const errExpectedGotIntel = "expected %q, got %q"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if im.tier != tt.expectTier {
				t.Errorf(errExpectedGotIntel, tt.expectTier, im.tier)
			}
			if im.provider == nil {
				t.Fatal("matched provider is nil")
			}
			if im.provider.Name != tt.expectName {
				t.Errorf(errExpectedGotIntel, tt.expectName, im.provider.Name)
			}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if im.provider.Name != "GoDaddy" {
				t.Errorf(errExpectedGotIntel, "GoDaddy", im.provider.Name)
			}
		})
	}
}
...
func TestGoldenRuleAnalyzeDNSInfrastructureEnterprise(t *testing.T) {
	...
	if provider != nameAmazonRoute53 {
		t.Errorf(errExpectedGotIntel, nameAmazonRoute53, provider)
	}

	assessment, _ := infra["assessment"].(string)
	if assessment != "Enterprise-grade DNS infrastructure" {
		t.Errorf(errExpectedGotIntel, "Enterprise-grade DNS infrastructure", assessment)
	}
