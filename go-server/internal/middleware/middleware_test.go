...
func TestCSRFPostWithoutCookie(t *testing.T) {
	...
	w := httptest.NewRecorder()
	req := httptest.NewRequest("POST", "/submit", strings.NewReader("csrf_token=sometoken"))
...
func TestCSRFPostWithInvalidSignature(t *testing.T) {
	...
	w := httptest.NewRecorder()
	req := httptest.NewRequest("POST", "/submit", strings.NewReader("csrf_token=faketoken"))
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
...
func TestRateLimitAllowsInitial(t *testing.T) {
	...
	result := limiter.CheckAndRecord("192.168.1.1", "example.com")
...
func TestAntiRepeatBlocksSameDomain(t *testing.T) {
	...
	result := limiter.CheckAndRecord("10.0.0.2", "example.com")
	if !result.Allowed {
