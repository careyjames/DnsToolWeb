...
- **Report Integrity hash preview**: Header format is `SHA-256: c82f✱✱✱✱ Verify Integrity ↓` — 4 hex chars + 4 star masks (&#x2731;) + "Verify Integrity" link with dotted underline. Uses `u-integrity-link` + `u-integrity-link-info` (cyan) on Engineer, `u-integrity-link-warning` (amber) on Executive. Dotted underline at 0.85 opacity → solid underline at full opacity on hover. Clear call-to-action, not passive metadata.
