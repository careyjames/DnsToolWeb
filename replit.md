...
- **CSS cache-busting**: After CSS edits, MUST: (1) run `npx csso` to minify, (2) bump `AppVersion` in `go-server/internal/config/config.go` so `staticVersionURL` generates a new `?v=` query string. Without the version bump, browsers serve stale CSS.
