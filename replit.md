...
- **Favicon & Brand Icon**: Athenian Owl of Athena logo (gold owl with Greek key border on dark background, blue ring accent). Source asset: `static/icons/icon-512x512.png`. Navbar uses `<img>` tag referencing `icon-192x192.png` at 24x24px with `rounded-circle` class. Favicons (16/32/48px PNGs, SVG, favicon.png) all generated from the 512px owl via ImageMagick resize. BIMI logo (`static/images/bimi-logo.svg`) embeds the owl as base64 PNG in SVG Tiny 1.2 PS profile. Cache-busting via `?v={{.AppVersion}}` query params. Dedicated `/favicon.ico` route serving 48x48 PNG for browser compatibility (GET+HEAD).
- **Footer Design**: Defined in `go-server/templates/_footer.html`. Centered layout with Owl of Athena image (120x120px, `u-rounded-circle` class) linking to it-help.tech, followed by tagline "We answer the BIG questions." and sub-tagline "DNS, same way since 1983." Below: copyright with IT Help San Diego Inc. link, then navigation links (Security Policy, Privacy Pledge, Changelog, Sources, Architecture, Confidence, Origin Story) separated by middot characters. Repository section shows GitHub and Codeberg links for both Web and CLI repos using inline SVG icons. All text uses Bootstrap `text-muted` and `small` classes on dark theme.
