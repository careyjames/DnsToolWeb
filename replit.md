...
- **Report Integrity hash preview**: Header format is `🔒 SHA-256: c82f✱✱✱✱ Verify` — fingerprint icon + 4 hex chars + 4 star masks (&#x2731;) as static text, then "Verify" as the sole clickable link. Wrapped in `u-integrity-preview` (inline-flex, nowrap) to keep it on one line. Only "Verify" gets dotted underline via `u-integrity-link`. Uses `u-integrity-link-info` (cyan) on Engineer, `u-integrity-link-warning` (amber) on Executive. Dotted → solid underline on hover.
- **OSINT positioning**: DNS Tool is explicitly an OSINT platform. All references should use "OSINT" terminology. Not a penetration test, not a PCI ASV scanner. Homepage meta, report meta, FAQ, llms.txt, and FEATURE_INVENTORY.md all aligned.
- **Homepage FAQ ordering**: PCI compliance FAQ is at position #2 (right after "What is a domain security audit?") to catch compliance seekers early. 16 FAQs total.
