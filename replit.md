...
- **Report Integrity hash preview**: Header format is `SHA-256: c82f✱✱✱✱ Report Integrity ↓` — 4 hex chars + 4 star masks (&#x2731;) + "Report Integrity" link. Engineer uses `text-info` (cyan), Executive uses `text-warning` (amber). Both have explicit `opacity:1` to override parent `u-opacity-70`. Font-size 0.75rem.
