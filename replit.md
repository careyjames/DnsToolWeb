...
## Repositories (Codeberg Canonical, GitHub Mirror)
- **Webapp**: `careybalboa/dns-tool-webapp` (Codeberg, canonical) → `careyjames/DnsToolWeb` (GitHub, read-only mirror)
- **Intel**: `careybalboa/dns-tool-intel` (Codeberg, canonical) → `careyjames/dnstool-intel` (GitHub, mirror)
- **CLI**: `careybalboa/dns-tool-cli` (Codeberg, archived) / `careyjames/dns-tool` (GitHub, archived)
- Push mirrors: Codeberg→GitHub, sync_on_commit + 8h interval. SonarCloud runs on GitHub mirror.
...
- **Webapp sync (Codeberg)**: `node scripts/codeberg-webapp-sync.mjs` — reads/writes `careybalboa/dns-tool-webapp` via Forgejo API.
...
- **CI**: Forgejo Actions on Codeberg (`.forgejo/workflows/ci.yml`), SonarCloud on GitHub mirror (`.github/workflows/sonarcloud.yml`)
